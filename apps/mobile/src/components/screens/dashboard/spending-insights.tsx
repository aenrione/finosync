  ...
  return (
    <View className="px-5 mb-6 mt-2">
      ...
      <View className="flex-row justify-between items-center">
        <Text className="text-2xl font-bold text-slate-900">Spending Insights</Text>
        <PeriodDropdown
      ...
      <View className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm mb-6">
        ...
        <View className="flex-row mb-5">
          ...
          <View className="flex-1 flex-row items-center">
            ...
            <View>
              <Text className="text-sm text-slate-500 mb-0.5">Total Spent</Text>
              <Text className="text-lg font-bold text-slate-900">{data && showAmount(data.total_spent, isVisible)}</Text>
            </View>
          </View>
          {/* Total Earned */}
          <View className="flex-1 flex-row items-center">
            <View className="w-10 h-10 rounded-full bg-slate-50 justify-center items-center mr-3">
              <Icon name="TrendingUp" size={20} className="text-green-700" />
            </View>
            <View>
              <Text className="text-sm text-slate-500 mb-0.5">Total Earned</Text>
              <Text className="text-lg font-bold text-slate-900">{data && showAmount(data.total_earned, isVisible)}</Text>
            </View>
          </View>
        </View>
        {/* Category Breakdown */}
        <View className="border-t border-slate-200 pt-4">
          <Text className="text-base font-semibold text-slate-900 mb-3">Top Categories</Text>
            ...
            {data && data.top_categories && data.top_categories.map((cat: Category & { amount: number }) => (
              <View key={cat.id} className="flex-row items-center justify-between">
                <View className="flex-row items-center">
                  ...
                  <Text className="text-sm text-slate-600">{cat.name}</Text>
                </View>
