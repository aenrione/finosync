...
describe("filterTransactions", () => {
  ...
  test("given 'Income' filter, includes credit transactions even if amount sign is inconsistent", () => {
    ...
    expect(result.map((transaction) => transaction.id)).toEqual([1, 4, 5]);
  });

  test("given 'Expenses' filter, includes debit transactions even if amount sign is inconsistent", () => {
    const result = filterTransactions(
      [
        ...sampleTransactions,
        makeTransaction({
    ...
    expect(result.map((transaction) => transaction.id)).toEqual([2, 3, 5]);
  });
