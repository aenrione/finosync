...
type TransactionsContextType = {
  ...
  fetchTransactionsByAccount: (accountId: number, page?: number, isRefresh?: boolean) => Promise<Transaction[]>
...
export function TransactionsProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchTransactions = useCallback(async (page: number = 1, isRefresh: boolean = false) => {
    try {
      ...
      const res = await fetchWithAuth(`/transactions?page=${page}&limit=${ITEMS_PER_PAGE}`)
      ...
      if (Array.isArray(responseData)) {
        ...
        transactions = Array.isArray(responseData.data) ? responseData.data : []
  ...
  const fetchTransactionsByAccount = useCallback(async (accountId: number, page: number = 1, isRefresh: boolean = false): Promise<Transaction[]> => {
    try {
      if (isRefresh) {
        setRefreshing(true)
      } else if (page === 1) {
      ...
      const res = await fetchWithAuth(`/transactions?account_id=${accountId}&page=${page}&limit=${ITEMS_PER_PAGE}`)
  ...
  const loadMore = useCallback(async () => {
    if (hasMore && !loadingMore) {
      if (currentAccountId) {
        await fetchTransactionsByAccount(currentAccountId, currentPage + 1, false)
      } else {
        await fetchTransactions(currentPage + 1, false)
      }
    }
  }, [hasMore, loadingMore, currentPage, fetchTransactions, fetchTransactionsByAccount, currentAccountId])

  const createTransaction = useCallback(async (formData: TransactionFormData): Promise<Transaction> => {
  ...
  const updateTransaction = useCallback(async (id: number, formData: Partial<TransactionFormData>): Promise<Transaction> => {
